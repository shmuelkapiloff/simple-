  ...
  if (error || !order) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <Link to="/orders" className="text-primary-600 hover:underline">חזרה להזמנות</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <nav className="text-sm text-gray-500 mb-6">
        <Link to="/orders" className="hover:text-primary-600">הזמנות</Link>
        ...
        {order.items.map((item: OrderItem, i: number) => (
          <div key={i} className="flex items-center gap-4 py-3 border-b last:border-b-0">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">כמות: {item.quantity} × ₪{item.price.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white rounded-xl border p-6 mb-6">
          ...
          <div className="space-y-3">
            {order.trackingHistory.map((entry: TrackingEntry, i: number) => {
              const es = statusMap[entry.status] ?? { label: entry.status, color: "bg-gray-100 text-gray-800" };
              return (
                <div key={i} className="flex items-start gap-3">
                  <div className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${i === 0 ? "bg-primary-500" : "bg-gray-300"}`} />
                  <div>
                    <p className="font-medium text-sm">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${es.color}`}>{es.label}</span>
                    </p>
                    {entry.message && <p className="text-sm text-gray-600 mt-0.5">{entry.message}</p>}
                    <p className="text-xs text-gray-400">{new Date(entry.timestamp).toLocaleString("he-IL")}</p>
                  </div>
                </div>
              );
            })}
